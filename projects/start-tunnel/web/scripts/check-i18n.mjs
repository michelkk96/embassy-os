...
// (not line-by-line) so usages prettier wrapped across multiple lines are still
// caught.
const KEY_PATTERNS = [
...
// Cross-language completeness. Translations are added incrementally, so an EMPTY
// dictionary is allowed (the pipe falls back to English). But once a language
// has ANY entry it must cover every id in en.ts — this forces a language, once
// started, to be finished rather than left half-translated.
...
for (const lang of otherLangs) {
  ...
  if (dictKeys.size === 0) continue // untranslated scaffold — skip
...
// other language ships a lazy file (content/{es,de,fr,pl}.ts). Like the
// dictionaries, an EMPTY translation file is allowed, but once it has any route
// it must cover every route in en.ts. Route keys look like `  '/route': `.
...
if (existsSync(helpContentDir) && existsSync(join(helpContentDir, 'en.ts'))) {
  ...
  for (const lang of helpLangs) {
    ...
    const langRoutes = [...readFileSync(path, 'utf-8').matchAll(ROUTE_RE)].map(
      m => m[1],
    )
    if (langRoutes.length === 0) continue // untranslated scaffold — skip

    const set = new Set(langRoutes)
    const missing = enRoutes.filter(r => !set.has(r))
    if (missing.length) {
