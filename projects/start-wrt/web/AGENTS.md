...
- **`appearance="positive|warning|…"`** on components, not `class="g-positive"`. When a raw color utility is unavoidable, use the `g-*` classes from this app's global `styles.scss` — never re-declare colors per component.
