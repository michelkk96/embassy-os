...
Some services can work with multiple backends — LND or Core Lightning for Lightning, File Browser or Nextcloud for media. An action lets the user choose, and each optional dependency's `enabled` function reads that choice to activate the selected backend.

## Solution

Create a selection action with `Value.select()` that lets the user choose between backends (e.g., LND vs CLN). Persist the choice to **`store.json` on the `startos` volume** — it is StartOS-level state, not part of the upstream service's config, so it must not be an invented key in the app's config file. Declare each backend with `sdk.Dependency.optional()` and read the choice in its `enabled` function. In `setupMain()`/`init`, read the same choice to conditionally mount the selected dependency's volumes and resolve _that_ backend's bridge address into the appropriate env vars or config keys.
