...
**`CONTRIBUTING.md` is being folded into `AGENTS.md`, scope by scope.** The target is one file per scope carrying both the contribution guide and the agent/dev operating rules. `projects/start-sdk/` is migrated — it has no `CONTRIBUTING.md`, and its `AGENTS.md` is the contribution guide. Every other scope still has the split. When you next do substantial work in a scope that hasn't migrated, fold its `CONTRIBUTING.md` into its `AGENTS.md`, delete it, and repoint every inbound link in the same change — don't leave dangling cross-references, and don't migrate scopes you aren't otherwise touching.
