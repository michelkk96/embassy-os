...
- **HTTP→HTTPS redirects on port 80.** StartTunnel now runs an HTTP→HTTPS redirect on port `80` of every public IPv4 it holds, so a plain `http://` request to an exposed service is answered with a redirect to the same host over `https://` instead of a connection error. These are **on by default** — every public IPv4 gets one on a fresh install and after an update — and reuse the same redirect handler the OS serves on its own TLS ports. Each address has a checkbox on the `Port Forwards` page (and `start-tunnel http-redirect list` / `set-enabled <ip> [--enabled]` on the CLI) to turn it off; your choice persists. A redirect is **mutually exclusive** with a port forward on that address's port 80: the forward wins, so forwarding port 80 (manually or automatically via PCP/UPnP) makes the redirect yield until the forward is removed. See the HTTP Redirects page in the docs.
