...
- **Regenerate bindings with `LC_ALL=C`.** `build.mk` builds `osBindings/index.ts` from a shell glob, so its line order follows `LC_COLLATE`. CI runs under `C`; regenerating under a UTF-8 locale reorders every export line, and CI's `Generated Artifacts` gate fails on the churn. If a regen shows only `index.ts`/`tunnel/index.ts` as modified, that's this — not a real change.
