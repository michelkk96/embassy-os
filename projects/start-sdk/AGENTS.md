...
- `dist/` — build output (generated; what publishes to npm). It bundles `@start9labs/start-core`, `eslint` and `typescript-eslint` (via npm `bundleDependencies`) so it stays self-contained — `lint.mjs` resolves ESLint out of the bundle, which is why packages need no eslint devDependency. All three are production `dependencies`, so they reach every consuming package as `inBundle` entries that nothing downstream can update: neither `overrides` nor `npm audit fix` reaches inside a bundled subtree, so an advisory against the bundled lint toolchain can only be cleared by an SDK release. **Container-runtime consumes the built `dist/`, not the source.**
