...
pub async fn pack(#[context] ctx: SdkContext, #[arg] path: Option<PathBuf>) -> Result<(), Error> {
    ...
    S9pkPacker::builder()
        ...
        .writer(&mut outfile)
        .license(
            File::open(path.join(manifest.assets.license_path())).await.with_ctx(|_| {
                (
        ...
        .icon(
            File::open(path.join(manifest.assets.icon_path())).await.with_ctx(|_| {
                (
                    crate::ErrorKind::Filesystem,
        ...
        .instructions(
            File::open(path.join(manifest.assets.instructions_path())).await.with_ctx(|_| {
                (
                    crate::ErrorKind::Filesystem,
        ...
        .docker_images(
            File::open(path.join(manifest.assets.docker_images_path())).await.with_ctx(|_| {
                (
                    crate::ErrorKind::Filesystem,
