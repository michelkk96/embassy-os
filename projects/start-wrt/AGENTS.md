...
- Don't run `make start-wrt-image` (full OpenWrt build) unsolicited — it pulls the openwrt submodule and takes hours. For backend work use `cargo build -p startwrt-core --bin startwrt`; for frontend work use `npm run start:wrt`. Use `make start-wrt-update STARTWRT_REMOTE=…` only when explicitly asked to deploy.
