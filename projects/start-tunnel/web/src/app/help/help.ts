...
import { I18N, i18nService, Languages } from '@start9labs/shared'
import HELP_EN from './content/en'

/** Whether the contextual help sidebar is expanded. Toggled from the header. */
export const HELP_OPEN = new InjectionToken('Help sidebar open status', {
  factory: () => signal(false),
})

/** The help-content key for the current screen/dialog (see ./content/en.ts). */
export const HELP_URL = new InjectionToken<string>('Help content key')
...
export class HelpService {
  ...
  private readonly loaded = signal<
    Partial<Record<Languages, Record<string, string>>>
  >({ en_US: HELP_EN })
  ...
  readonly content = computed<Record<string, string>>(() => {
    ...
    for (const route of Object.keys(HELP_EN)) {
      out[route] =
        (lang !== 'en_US' && translation?.[route]) || HELP_EN[route] || ''
    }
    return out
  })
