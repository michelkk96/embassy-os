...
/**
 * In-dialog contextual help. Dialogs do not change the router URL, so the
 * URL-keyed sidebar (`aside.ts`) can't reach them — instead a dialog opts in
 * with `hostDirectives: [ModalHelp]` + `providers: [provideHelp('/key')]` and
 * this renders a floating "Help" toggle that pops the matching content.
 */
@Component({
  template: `
