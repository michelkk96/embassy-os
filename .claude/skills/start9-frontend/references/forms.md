...
<form tuiForm="m" [formGroup]="form" (submit.prevent)="save()">
  ...
  <footer>
    <button tuiButton appearance="flat" type="button" (click)="context.$implicit.complete()">
      Cancel
    </button>
...
- **Submit**: prefer an **enabled** submit that runs
  `tuiMarkControlAsTouchedAndValidate(this.form)` and bails if invalid — from review: *"a good
  UX pattern is to not disable it… so that I can see all the fields I forgot to type in."*
